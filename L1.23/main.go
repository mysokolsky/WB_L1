...
func dropOneIndex(index int, mas *[]int) error {
	...
	// tempParam := (*mas)[len(*mas)-1]                   // временная переменная для сохранения значения последнего элемента перед его занулением
	// clear((*mas)[len(*mas)-1:])                        // для версий go 1.21 и выше. Зануляет последний элемент, автоматически определяя его тип
	...
	// if index < len(*mas)-1 {                           // условие для случая пустого массива или удаления конечного элемента
	// 	(*mas)[len(*mas)-1] = tempParam // присваиваем сохранённое ранее значение последнему элементу
	// }
	...
	*mas = slices.Delete(*mas, index, index+1) // для выполнения необходимо импортировать библиотеку slices
