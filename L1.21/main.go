...
// Существует два варианта реализации паттерна Адаптер:

// Объектный адаптер (object adapter):
// использует композицию (адаптер содержит экземпляр адаптируемого класса).

// Классовый адаптер (class adapter):
// использует множественное наследование
// (не применимо в Go, поскольку Go не поддерживает наследование классов, только интерфейсы и встраивание).

// !!! В Go паттерн обычно реализуется через объектный адаптер с использованием интерфейсов и композиции.

// В классическом исполнении в GO адаптер имеет вид:
...
//
// Минусы:
// 1. за счёт добавки дополнительного слоя усложняет читабельность кода
// 2. зависит от исходного кода. Если он изменится, то адаптер тоже придётся переписывать

// Не стоит применять:
// 1. Интерфейсы уже совместимы
// 2. Когда нужно полностью переписывать исходный код
