...
loop3:
	for i := 0; ; i++ {
		...
		case <-interruptCh: // при получении сигнала системы и закрытии канала
			...
			break loop3
		case <-timer:
			fmt.Print("\nГорутина №6: завершение по таймеру")
			close(ch6) // закрываем канал данных
			break loop3
		default:
...
func runtimeExit(name string, ch7 chan failObj) {
	...
	defer func() { ch7 <- failObj{} }() // пытаемся записать объект в закрытый канал. Возникает ещё одна паника
	defer close(ch7)                    // закрываем канал
...
func runtimeExitRun() {
	...
	fmt.Println("Горутина успешно завершилась без паник, поскольку принудительное завершение горутины через runtime.Goexit() убило горутину вместе с паниками")
}
