...
func someFunc() {
	...
	justString = v[:100]           // Это ссылка на строку-исходник v. Пока программа будет работать, первые 100 ячеек( c 0 по 99ю) будут использоваться, остальные (1024 - 100 = 924 байта) будут просто зарезервированы, но не использованы. То есть программа потребляет излишние ресурсы памяти.
}
